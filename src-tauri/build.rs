fn main() {
    ...
    //
    // Note: #[cfg(feature)] and #[cfg(target_family)] check at compile time against
    // the *host* in build.rs. We add target_family = "unix" so these functions don't
    // compile on Windows hosts (std::os::unix doesn't exist there). The runtime
    // CARGO_CFG_TARGET_OS check further gates on the *target* OS for cross-compilation.
    #[cfg(all(feature = "input-syphon", target_family = "unix"))]
    if target_os == "macos" {
