...
export default function PixelMapSection({
  ...
  const handlePatternChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    update({ pattern: e.target.value as PixelMapPattern });
  };
  ...
  return (
    <div className="px-2 py-2 space-y-2">
      ...
      <button
        ...
        className="text-[10px] text-zinc-500 hover:text-zinc-400 flex items-center gap-1 uppercase tracking-wider"
      >
        <span
        ...
        <div className="space-y-2">
          ...
          {enabled && pixelMap && (
            <>
              {/* Pattern picker */}
              <div>
                <span className="block text-[10px] text-zinc-500 mb-0.5">Pattern</span>
                <select
                  value={pixelMap.pattern}
                  onChange={handlePatternChange}
                  className="w-full text-[11px] bg-zinc-800 text-zinc-200 border border-zinc-700 rounded px-1.5 py-1 outline-none focus:border-indigo-500"
                >
